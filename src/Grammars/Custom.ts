...
function getSubItems(tmpRules: IRule[], seq: IToken, parentName: string, parentAttributes: any) {
  ...
  seq.children.forEach((x, i) => {
    ...
    const pinned = preDecoration == '~' ? 1 : undefined;
...
function createRule(tmpRules: IRule[], token: IToken, name: string, parentAttributes: any = undefined) {
  const attrNode = token.children.filter((x) => x.type == 'Attributes')[0];
  ...
  const bnf = token.children
    .filter((x) => x.type == 'SequenceOrDifference')
    .map((s) => getSubItems(tmpRules, s, name, parentAttributes ? parentAttributes : attributes));
  ...
  if ('pin' in attributes) {
    const num = parseInt(attributes['pin']);
    if (!isNaN(num)) {
      rule.pinned = num;
    }
...
export function getRules(source: string, parser: _Parser = defaultParser): IRule[] {
  ...
  const attrNode = ast.children.filter((x) => x.type == 'Attributes')[0];
